...
class ProtoTEx_BERT(torch.nn.Module):
    def __init__(
        self,
        num_prototypes,
        n_classes,
        ...
        self.one_by_sqrt_bertoutdim = 1 / torch.sqrt(
            torch.tensor(self.bert_out_dim).float()
        )
        ...
        self.prototypes = torch.nn.Parameter(
            torch.rand(self.num_protos, self.max_position_embeddings, self.bert_out_dim)
        )
    ...
    def forward(
        ...
        if use_p3:
            ...
            l_p3 = self.one_by_sqrt_bertoutdim * torch.mean(
