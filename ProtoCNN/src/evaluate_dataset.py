...
def load_preprocess(dataset_dir, tokenizer_name="bert-base-uncased"):
    ...
    try:
        all_datasets = load_dataset(
            "csv",
            data_files={
                file_name[: file_name.find(".csv")]: os.path.join(
    ...
    for dataset_name in all_datasets.keys():
        yield dataset_name, DataLoader(
            all_datasets[dataset_name].with_format("torch"), batch_size=1
        )
...
if __name__ == "__main__":
    ...
    for name, loader in load_preprocess(
        tokenizer_name=args.tokenizer_name, dataset_dir=args.dataset_dir
    ):
        similarities = []
