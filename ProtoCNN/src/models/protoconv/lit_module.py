...
class ProtoConvLitModule(pl.LightningModule):
    ...
    def __init__(self, vocab_size, embedding_dim, fold_id=0, lr=1e-3, static_embedding=False,
        ...
        self.valid_acc = torchmetrics.Accuracy("binary")
    ...
    def learning_step(self, batch, acc_score):
        ...
        cross_entropy = self.loss(outputs.logits, torch.tensor(batch["label"], dtype=torch.float32).to(self.device))
        
        _, dce_loss = self.dce_loss(self.prototypes.prototypes, outputs)
        dce_loss = F.nll_loss(F.log_softmax(dce_loss, dim=1))
        ...
        # return LossesWrapper(loss, dce_loss, clustering_loss, separation_loss, l1, accuracy)
    ...
    def log_all_metrics(self, stage, losses: LossesWrapper):
        ...
        self.log(f'{stage}_ce_{self.fold_id}', losses.cross_entropy, prog_bar=False, batch_size=BATCH_SIZE)
        self.log(f'{stage}_clst_{self.fold_id}', losses.clustering_loss, prog_bar=False, batch_size=BATCH_SIZE)
        self.log(f'{stage}_sep_{self.fold_id}', losses.separation_loss, prog_bar=False, batch_size=BATCH_SIZE)
        self.log(f'{stage}_l1_{self.fold_id}', losses.l1, prog_bar=False, batch_size=BATCH_SIZE)
        self.log(f'{stage}_acc_{self.fold_id}', losses.accuracy, prog_bar=True, on_step=False, on_epoch=True, batch_size=BATCH_SIZE)
    ...
    def dce_loss(self, prototypes, outputs):
        # prototypes : (num_prototypes, conv_filters, 1) -> (1, num_proto, conv_filters)
        # latent_space: (bs, conv_filters, conv_out_dim=128)
        # logits : (bs, )
        embed()
        features_square = torch.sum(torch.pow(outputs.latent_space, 2), 1, keepdim=True)
        centers_square = torch.sum(torch.pow(prototypes, 2), 0, keepdim=True)
        features_into_centers = 2 * torch.matmul(outputs.latent_space, (prototypes))
